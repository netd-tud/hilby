...
//zoom)
    ...
    (value: number | ((current: number) => number), maybeCenter?: position) => {
      ...
      if (container) {
        setTransform(({ x, y, zoom }) => {
          ...
          //console.log("setZoom", clampX(x + ((center.x - x) * (zoom - newZoom)) / zoom),
          //clampY(y + ((center.y - y) * (zoom - newZoom)) / zoom),
          //newZoom)
          return {
  ...
  const movePanZoom = useCallback(
    (pointers: position[]) => {
      ...
      if (state.isPanning) {
        ...
        //console.log("pre", dx, dy)
        dx /= l;
        dy /= l;
        //console.log("post", dx, dy)
        const scale =
          l === 2
  ...
  const onTouchMove = useCallback(
    (event: React.TouchEvent) => {
      ...
      if (updateRef.current === null) updateRef.current = event.timeStamp;

      if (event.timeStamp <= updateRef.current) return;

      updateRef.current = event.timeStamp;
